...
effectsize0 <- function(x, y, design, method, coverage) {
  ...
  if (method=="noncentral") {
    ...
    tll_d = qtAppr(0.5-coverage/2, df=nu, ncp=lambda_d) ## maybe use qtAppr with the same syntax from the DPQ package
    ...
    tll = qtAppr(0.5-coverage/2, df=nu, ncp=0)
    tul = qtAppr(0.5+coverage/2, df=nu, ncp=0)
    # Cohen's d 95%CI
